        ...
        <div class="row py-5">
                ...
                <select id="marketing" class="form-select " required aria-label="Default select example">
                    <option class="text-end" disabled selected>اختر الدورة المراد التسويق لها</option>
                        @foreach ($courses as $course)
                            <option class="text-end" data-link="{{ route('checkout',$course->id) }}/?buyBy={{ Auth::id() }}">{{ $course->name }}</option>
                        @endforeach
                </select>
            </div>
            <div class="col-md-12 userlink">
                ...
                <div class="userLink mt-4 position-relative">
                    <input type="text" class="form-control text-start" id='linkCourse' readonly name="userlink" value="unique user link">
                    <i class="fa-regular fa-copy cursor-pointer main-color fs-4 position-absolute" id="copyIcon"></i>
                </div>
            </div>
            <div class="col-md-12 mt-4">
                <h2 class="h5 fw-bold py-1">الرصيد الحالي : <span id="moneyWallet">{{ Auth::user()->wallet }}</span> جنيه</h2>
                <div class="mb-5" id="moneyHaveing">
                    <div class="form-outline mb-3" id="walletData">
                        <input type="text" class="form-control " placeholder=' ادخل رقم الهاتف' id="phoneNumer" required
                            max="11" min="11" />
                    </div>
                    <div class="form-outline mb-3" id="walletData">
